...
INFO:
    Query-selected-attention NCE loss (encoder side): extracts multi-scale
    features from the cover/encoded images and applies PatchNCE loss to
    encourage the encoder to preserve cover-image content.
"""
...
class QS_Attn_Loss(tf.keras.layers.Layer):
    def __init__(self, args, net, n_layers, kind_net=""):
        super(QS_Attn_Loss, self).__init__()
    ...
    def call(self, cover, encoded):
        
 
        feat_k = self.featureExtractor(cover)
        feat_q = self.featureExtractor(encoded)
        ...
        for f_q, f_k in zip(feat_q_pool, feat_k_pool):
            ...
            total_nce_loss += np.mean(loss) # tf.math.reduce_mean
