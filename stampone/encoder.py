...
INFO:
    CLI entry point: reads a folder of original images, embeds a message
    (or random bits) into each with the StampOne encoder, and writes the
    encoded images, encoded region crops, and 2D message previews to
    --save_dir. Run with: stampone-encode --original_images <dir> ...
"""


import tensorflow as tf
from  stampone.tools_nvidia.utils import Configure_GPU
from stampone.FarhadCV.Tools import tcolors, bcolors, estimator, read_files, mkdirfile
import cv2
import numpy as np
from stampone.tools_encoder.tools_TFLite_encoder import BCH_Generator
...
def encoder_router(args, devices):
    ...
    with tf.device(devices[0]):
        from stampone.tools_encoder.tools_TFLite_encoder import TFLite_Encoder_Loader
        StampOne_Encoder = TFLite_Encoder_Loader()

    #######################################################
    ######        Load Detection Router         ######
    #######################################################
    from stampone.tools_detection.main import Detection_Models
