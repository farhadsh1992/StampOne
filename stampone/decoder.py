...
INFO:
    CLI entry point: reads a folder of encoded images, recovers the
    hidden message from each with the StampOne decoder, and writes the
    decoded messages to a CSV in --save_dir. Run with:
    stampone-decode --encoded_images <dir> ...
"""
...
import argparse
from stampone.tools_decoder.tools_TFLite_decoder import read_message
import cv2
import pandas as pd
...
def decoder_router(args:argparse.ArgumentParser, devices:list):
    ...
    with tf.device(devices[0]):
        from stampone.tools_decoder.tools_TFLite_decoder import TFLite_Decoder_Loader
